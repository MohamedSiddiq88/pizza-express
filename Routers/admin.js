...
router.post("/signup",async(req,res)=>{
    try {
        ...
        const user=await getAdmin(req.body.email);
        if(!user){
            const hashedPassword=await bcrypt.hash(req.body.password, salt)
            const hashedUser=await {...req.body,password: hashedPassword}
            const result= await addAdmin(hashedUser);
...
router.post("/checkmail",async(req,res)=>{
    ...
    const link = `https://ornate-duckanoo-99a24c.netlify.app/resetpassword?token=${randomString}&role=admin`;
